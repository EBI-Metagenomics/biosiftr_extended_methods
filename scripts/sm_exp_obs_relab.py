...
def exp_parser( ref_abun, mock_prefix ):
    exp_dict = {}
...
def obs_parser( obs_table, exp_dict, mock_prefix ):
    ...
    exp_used = []
    with open('sm_relab_obs_exp.tsv', 'w') as output_file:
...
def main():
    ...
    parser.add_argument(
        "--sp_relab",
        type=str,
        help="Sourmash relative abundance table at species level (sm_species_uw.tsv)",
        required=True,
    )
    parser.add_argument(
        "--dataset_type",
        type=str,
        help="Complexity data type: low or high",
        required=True,
    )
    parser.add_argument(
        "--synth_data",
        type=str,
        help="Synthetic dataset synth_[1..20]",
        required=True,
    )
    ...
    ### Calling functions
    ( exp_dict ) = exp_parser( ref_abun, args.synth_data )
    obs_parser( args.sp_relab, exp_dict, args.synth_data )
