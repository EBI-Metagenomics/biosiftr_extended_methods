...
dRep dereplicate -g genomes_dir/* -p ${cpus} -pa 0.9 -sa 0.95 -nc 0.30 -cm larger -comp 50 -con 5 drep_output
...
# Functional annotation of all genomes according with the dRep table drep_output/data_tables/Cdb.csv
...
# Second step of functional annotation on representative genomes only
...
# Move the corresponding genome_prokka/genome.gff files to a new directory called as the representative genome according with the dRep table drep_output/data_tables/Cdb.csv
    panaroo \
    -t ${cpus} \
    -i rep_genome/*.gff \
    -o rep_genome_panaroo \
    --clean-mode strict \
    --merge_paralogs \
    --core_threshold 0.90 \
    --threshold 0.90 \
...
metadata_custom.py --input drep_output/data_tables/Cdb.csv --output genomes-all_metadata.tsv

# Tidying up the directory structure

file_organiser_custom.sh 

```

A catalogue built this way can be used to generate the Shallowmapping databases as described above.
